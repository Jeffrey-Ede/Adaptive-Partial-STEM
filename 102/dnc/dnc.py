...
class Components(snt.AbstractModule):
  ...
  def __init__(self,
    ...
    with self._enter_variable_scope():
      self.controller = snt.DeepRNN([snt.LSTM(**controller_config), snt.LSTM(**controller_config)])
...
class DNC(snt.RNNCore):
  ...
  def _build(self, inputs, prev_state):
    ...
    if self._is_new:
        ...
        if FLAGS.is_prev_position_input:
            ...
            if self._output_size_features == 1 and not FLAGS.is_advantage_actor_critic: #If critic
    ...
    elif self._output_size_features != 1:#If not critic
  ...
  def _neural_machine(
    ...
    if self._output_size_features != 1: #If not critic
  ...
  def initial_state(self, batch_size, dtype=tf.float32):
    ...
    return DNCState(
        ...
        output=tf.ones([batch_size, self._output_size_features])/np.sqrt(2),
        position=tf.ones([batch_size, FLAGS.num_actions])/2,
  ...
  def obs(self, actions, starts):
      ...
      if hasattr(tf, 'ensure_shape'):
          x = tf.ensure_shape(x, [FLAGS.batch_size, FLAGS.step_size])
      else:
          x = tf.reshape(x, [FLAGS.batch_size, FLAGS.step_size])
      return x
