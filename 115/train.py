...
tf.flags.DEFINE_bool("is_minmax_reward", True, "True to use highest losses for actor loss.")
...
def main(unused_argv):
    ...
    unclipped_losses = generator_losses
    ...
    if FLAGS.is_target_generator and not FLAGS.is_infilled:
        ...
        if FLAGS.is_minmax_reward:
            clipped_output = tf.clip_by_value( target_generation, -1, 1)
            errors =  (clipped_output - replay_full_scans_ph)**2
        ...
        if FLAGS.is_minmax_reward:
            clipped_output = tf.clip_by_value( generation, -1, 1)
            errors =  (clipped_output - replay_full_scans_ph)**2
            losses = tf.reduce_max( average_filter(errors), reduction_indices=[1,2,3] )
        else:
