...
tf.flags.DEFINE_integer("replay_size", 25_000, "Maximum examples in ring buffer.")
...
tf.flags.DEFINE_bool("is_ornstein_uhlenbeck", False, "True for O-U exploration noise.")
...
tf.flags.DEFINE_float("over_edge_penalty", 0.1, "Penalty for action going over edge of image.")
...
tf.flags.DEFINE_bool("is_true_action", False, "True to replace replay actor actions with true actions.")

tf.flags.DEFINE_bool("is_perturb_to_true_action", False, "True to perturb replay actor actions to true actions.")

tf.flags.DEFINE_integer("start_iter", 100, "Starting iteration")
...
@tf.custom_gradient
def backprop_to_true_actions(*args):
    actions, replay_actions = args[0], args[1]

    def grad(*dy):
        print("DY", dy)
        return dy[1], None

    return (actions, replay_actions), grad
...
def main(unused_argv):
    ...
    if FLAGS.is_true_action:
        _, actions = backprop_to_true_actions(actions, replay_actions)

    if FLAGS.is_perturb_to_true_action:
        actions += tf.stop_gradient(replay_actions - actions)
    ...
    blurred = blur(replay_full_scans_ph)
    outputs = {
    ...
    with tf.Session(config=config) as sess:
        ...
        for iter in range(start_iter, train_iters):
            ...
            sampled_actions, sampled_observations, replay_sampled_full_scans = sess.run(
            ...
            #Train
            if iter < 100:
                step_info, step_outputs = sess.run([info, outputs], feed_dict=feed_dict)
          
                for k in step_outputs:
                    save_loc = FLAGS.model_dir + "final_" + k + str(iter)+".tif"
