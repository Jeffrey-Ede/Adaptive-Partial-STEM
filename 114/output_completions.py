...
def main(unused_argv):
    ...
    val_full_scans = tf.placeholder(tf.float32)
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(0, images.shape[0]//FLAGS.batch_size):

            print(f"Iter {i} of {images.shape[0]//FLAGS.batch_size}")

            sampled_full_scans = images[i*FLAGS.batch_size:(i+1)*FLAGS.batch_size]


            feed_dict = {
                is_training_ph: np.bool(False),
                val_full_scans: sampled_full_scans
                }

            sampled_actions, sampled_observations = sess.run([val_actions, val_observations], feed_dict=feed_dict)
            ...
            generated_images = sess.run(generation, feed_dict=feed_dict)
            outputs[i*FLAGS.batch_size:(i+1)*FLAGS.batch_size] = generated_images

        outputs = outputs[:images.shape[0]-images.shape[0]%FLAGS.batch_size]
        print(outputs.shape)

        np.save(FLAGS.model_dir+"generator_outputs.npy", outputs)
